...
class CIFAR10DVS(EventsFramesDatasetBase):
    ...
    def download_and_extract(download_root: str, extract_root: str):
        for key in resource.keys():
            ...
            if os.path.exists(file_name):
                ...
                print(f'{file_name} exists, to download and extract...')
                utils.download_and_extract_archive(
                    resource[key][0],
                    download_root,
                    extract_root,
    ...
    def read_bin(file_name: str):
        ...
        # ### There may have a little problem in the original author;s code, so it does not return [t x y p] directly

    @staticmethod
    def create_frames_dataset(
            events_data_dir: str, frames_data_dir: str, frames_num: int,
            split_by: str, normalization: str or None):
    ...
    def __init__(self, root: str, train: bool, split_ratio=0.9, use_frame=True,
                 frames_num=10, split_by='number', normalization='max', transform=None):
        '''
